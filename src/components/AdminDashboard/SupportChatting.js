...
import {  AlertCircle, SendHorizontal,Image,User,CircleX,Eye, } from 'lucide-react';
      ...
      {false && (
        ...
        <div className='w-full h-full max-h-[calc(100vh-20px)] overflow-hidden p-4 bg-[#111827] border border-gray-700 rounded-lg shadow shadow-[#2ab6e4] flex relative'>
            ...
            <div className='w-full h-[calc(100vh-90px)] overflow-y-auto flex-col pr-4
             py-2 ' >
                {Sessions?.length > 0 ? Sessions.filter((session) => session?.user?.fullName?.toLowerCase().includes(SearchTerm.toLowerCase())).map((session, index) => (
                    ...
                    <span className='py-2 px-3 text-wrap bg-[#] text-white rounded-lg w-full flex items-center gap-2'>
                       <User/> {session?.user?.fullName ?? 'No message content'}
        ...
        <div className='h-full w-full flex flex-col '>
            ...
            <div className='flex items-center text-white gap-10'>
                <span className='text-gray-400 text-base text-nowrap h-full'>User Details:</span>
                
            {sessionActive && <div className='flex flex-col items-start gap-1'>
                <p className='text-gray-400 text-ellipsis overflow-hidden text-wrap leading line-clamp-2 text-xs'><span className='text-white'>Name :</span> {Sessions?.find((session) => session?._id === sessionActive)?.user?.fullName ?? 'No name available'}</p>
                <p className='text-gray-400 text-ellipsis overflow-hidden text-wrap leading line-clamp-2 text-xs'><span className='text-white'>Email :</span> {Sessions?.find((session) => session?._id === sessionActive)?.user?.email ?? 'No email available'}</p>
            </div>}
            </div>
